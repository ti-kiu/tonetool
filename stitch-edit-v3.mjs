import { stitch, Screen } from '/root/node_modules/@google/stitch-sdk/dist/src/index.js';

async function main() {
  try {
    console.log('Loading project and screen...');
    
    // 获取 client
    const client = stitch.client || stitch._client;
    console.log('Client ready');

    // 直接用已知的 project ID 和 screen ID 创建 Screen 对象
    const projectId = '14230594293766437443';
    const screenId = 'c3e8c740ddc84934b4b27a19bf2b4376';
    
    // 从 stitch 获取 client
    const stitchClient = stitch.project(projectId).client;
    
    const screen = new Screen(stitchClient, { 
      projectId: projectId, 
      id: screenId 
    });
